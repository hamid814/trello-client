...
const MoveCard = () => {
  ...
  useEffect(() => {
    ...
    setDestPos(
      getList(currentBoardId, currentListId).items.findIndex(
        (i) => i.id === currentCard.id
      ) + 1
    );
  ...
  return (
    <div className="move-card-modal text-85">
      {/* board selectList */}
      <section>
        <div className="p">board</div>
        <select
          ref={boardsSelect}
          value={destBoardId}
          onChange={onBoardDestChange}
          className="mb"
        >
          {boards.map(
            (board) =>
              board.lists.length !== 0 && (
                <option key={board._id} value={board.id}>
                  {board.title}
            ...
            {destBoardId &&
              ...
              getList(destBoardId, destListId).items.map((item, index) => (
                <option key={item.id} value={index + 1}>
                  {index + 1}
                  {item.id === currentCard.id && ' (current)'}
                </option>
              ))}
            {destBoardId && destListId && destListId !== currentListId && (
              <option value={getList(destBoardId, destListId).items.length + 1}>
                {getList(destBoardId, destListId).items.length + 1}
              </option>
            )}
          </select>
        </section>
